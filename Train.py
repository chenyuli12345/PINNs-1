...
def optimizer(args, dnn):
    if args.optimizer == 'LBFGS':
        optimizer = torch.optim.LBFGS(
        dnn.parameters(), #要优化的参数，这里返回的是一个生成器，包含了self.dnn中的所有参数（神经网络权重与偏置）
        lr=1.0,  #学习率设置为1
        ...
        optimizer = torch.optim.SGD(dnn.parameters(), lr=0.01, momentum=0.9)

    else:
        print("Unknown optimizer")
        exit(0) #用于终止程序的执行，并返回一个推出代码，这里0表示正常退出，退出代码为0
